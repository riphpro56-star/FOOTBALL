...
export default async function handler(req, res) {
  ...
  try {
    ...
    const videos = items.slice(0, 24).map(item => ({
    ...
    res.status(200).json({ source: "ScoreBat", count: videos.length, videos });
  } catch (err) {
    res.status(500).json({ error: "Could not load videos", detail: err.message });
  }
}
