...
const els = {
  ...
  modalVideoFrame: document.getElementById("modalVideoFrame")
};
...
function renderLiveCards() {
  ...
  if (!live.length) {
    ...
    els.liveCards.innerHTML = upcoming.length
      ...
      : `<div class="empty">No live famous matches right now.</div>`;
    return;
  }
...
function renderMatches() {
  ...
  if (!visible.length) {
    els.matchList.innerHTML = `<div class="empty">${state.mode === "featured" ? "No famous league matches found. Tap View all fixtures." : "No matches found."}</div>`;
    els.loadMoreBtn.style.display = "block";
    els.loadMoreBtn.textContent = state.mode === "featured" ? "View all fixtures ›" : "Back to famous matches";
    return;
  }
  ...
  els.loadMoreBtn.style.display = "block";
  els.loadMoreBtn.textContent = state.mode === "featured"
    ? "View all fixtures ›"
    : (list.length > state.visibleCount ? "Load more matches ›" : "Back to famous matches");
}
...
function renderVideos() {
  ...
  els.videoRow.innerHTML = vids.map((v, i) => `
    <button class="video-card" data-video-index="${i}" type="button">
  ...
  els.videoRow.querySelectorAll(".video-card").forEach(btn => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.videoIndex);
      openVideoModal(state.videos[index]);
    });
  });
}
...
function openVideoModal(video) {
  ...
  els.modalVideoTitle.textContent = video.title || "Match Highlights";

  if (video.embed) {
    els.modalVideoFrame.innerHTML = video.embed;
  } else if (video.url) {
    els.modalVideoFrame.innerHTML = `
      <iframe src="${escapeHTML(video.url)}" allowfullscreen loading="lazy"></iframe>
    `;
  } else {
    els.modalVideoFrame.innerHTML = `<div class="empty">Video not available.</div>`;
  }

  const iframe = els.modalVideoFrame.querySelector("iframe");
  if (iframe) {
...
async function loadMatches() {
  ...
  els.liveCards.innerHTML = `<div class="skeleton-card"></div><div class="skeleton-card"></div>`;

  try {
    const res = await fetch(`/api/matches?date=${state.selectedDate}&mode=${state.mode}`);
    ...
    showToast(state.mode === "featured" ? "Famous leagues loaded" : "All matches loaded");
  } catch (err) {
...
function setupEvents() {
  ...
  document.querySelectorAll(".link-btn[data-filter='live']").forEach(btn => {
    btn.addEventListener("click", () => {
      state.mode = "all";
      state.status = "live";
      document.querySelectorAll(".pill").forEach(b => b.classList.toggle("active", b.dataset.status === "live"));
      loadMatches();
  ...
  els.videoBackdrop.addEventListener("click", closeVideoModal);
